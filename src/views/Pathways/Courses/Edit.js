...
import { getCourseEditFormStructure } from "./form"
...
const PathwayCourseEdit = ({ rightPaneLoading, actions, match, theme }) => {
  ...
  useEffect(() => {
    const fetchallCourses = async () => {
      ...
      const responseOfOpenedCourse = await ngFetch(
        `/pathways/${pathwayId}/courses/${pathwayCourseId}`
      )
  ...
  const onSubmit = async data => {
    {
      find(coursesAlreadyPresent, {
        course_id: parseInt(data.course_id, 10),
      }) && enqueueSnackbar("Course is already there.", { variant: "failed" })
    }
    ...
    actions.addOrEditCourse({
      pathwaysCourse: response.updatedpathwayCourse,
      pathwaysCourseId: response.updatedpathwayCourse.id,
    })
  ...
  return (
    <RightPaneWithTitle
      title="Edit Course"
      closeLink={`/pathways/${pathwayId}/courses`}
    >
      <FormBuilder
        structure={getCourseEditFormStructure(course, allCourses)}
        onSubmit={onSubmit}
...
const mapDispatchToProps = dispatch => ({
  actions: bindActionCreators(
    { setRightPaneLoading, setCourseToView, addOrEditCourse },
    dispatch
  ),
})
