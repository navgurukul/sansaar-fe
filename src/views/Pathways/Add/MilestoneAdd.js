...
import { setAddOrEditMilestone } from '../store';
...
import { getMilestoneAddFormStructure } from '../Forms/milestoneFormStructure';
...
const MilestoneAdd = ({ actions, match }) => {
  ...
  const onSubmit = async (data) => {
    ...
    actions.setAddOrEditMilestone({milestone: response.pathway[0], milestoneId:response.pathway[0].id});
...
const mapDispatchToProps = (dispatch) => ({
  actions: bindActionCreators({ setAddOrEditMilestone,setRightPaneLoading }, dispatch),
});
